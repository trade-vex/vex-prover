use crate::{
    components::{
        ...
        trace_utils::{
            add_interaction_col, add_less_than_interaction_col,
            add_leaf_interaction_col_batched, add_merkle_interaction_col_batched, MerkleStep,
        },
...
pub fn interaction_trace<S: OrderSide>(
    ...
    //   1. Low tree - original path
    //   2. Low tree - updated path
    //   3. New tree - original path (inactive leaf)
    //   4. New tree - updated path
    //
    // For each proof:
    ...
    //
    // Batching strategy:
    //   - Batch all 4 leaf hashes into 1 column (column 4)
    //   - Batch all 4 merkle steps at each level into 1 column per level (columns 5-24)

    // Batch all 4 leaf hash operations together
    let leaf_batch = vec![
        // Proof 1: Low tree - original leaf
        {
            let leaf_state: [&Vec<PackedBaseField>; N_STATE] = low_leaf[0..N_STATE].try_into().unwrap();
            let leaf_hash = low_merkle_path[0];
            let elements_array: [&Vec<PackedBaseField>; N_STATE + N_HASH] = flatten!(leaf_state, leaf_hash);
            let elements = elements_array.to_vec();
            (elements, PackedSecureField::one())
        },
        // Proof 2: Low tree - updated leaf
        {
            let leaf_state: [&Vec<PackedBaseField>; N_STATE] = updated_low_leaf[0..N_STATE].try_into().unwrap();
            let leaf_hash = updated_low_merkle_path[0];
            let elements_array: [&Vec<PackedBaseField>; N_STATE + N_HASH] = flatten!(leaf_state, leaf_hash);
            let elements = elements_array.to_vec();
            (elements, PackedSecureField::one())
        },
        // Proof 3: New tree - original leaf (inactive)
        {
            let leaf_state: [&Vec<PackedBaseField>; N_STATE] = inactive_leaf[0..N_STATE].try_into().unwrap();
            let leaf_hash = merkle_path[0];
            let elements_array: [&Vec<PackedBaseField>; N_STATE + N_HASH] = flatten!(leaf_state, leaf_hash);
            let elements = elements_array.to_vec();
            (elements, PackedSecureField::one())
        },
        // Proof 4: New tree - updated leaf
        {
    ...
    add_leaf_interaction_col_batched(&mut logup_gen, &[leaf_batch], is_real, log_size, poseidon_elements);
    ...
    for level in 0..MERKLE_HEIGHT {
        ...
        // Proof 1: Low tree - original path
        ...
        // Proof 3: New tree - original path
        level_batch.push(MerkleStep {
    ...
    add_merkle_interaction_col_batched(&mut logup_gen, &merkle_batches, is_real, log_size, poseidon_elements);
