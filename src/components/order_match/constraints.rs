...
impl<S: OrderSide, T: OrderMatchType> FrameworkEval for MatchEval<S, T> {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        //   - Aggressive: less_than, match, 4×(leaf+20 merkle), instruction = 87 relations
        //   - Passive: match, 4×(leaf+20 merkle), instruction = 86 relations
        //
        // Batching maps to:
        //   - Aggressive: 24 columns (less_than, match, leaf_batch, 20 merkle_batch, instruction)
        //   - Passive: 23 columns (match, leaf_batch, 20 merkle_batch, instruction)
        ...
        // For 4 merkle proofs, each adds 1 leaf + 20 merkle steps
        for _ in 0..4 {
