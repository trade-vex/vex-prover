...
///  1) Clearing the target leaf (making it inactive)
///  2) Updating the prev leaf's "next" value to point to deleted leaf's next
///  The constraints must ensure that the leaf to be deleted is active and the updates are done correctly.
///
...
///   4. Assert that the leaf to be deleted is active (has active bit set) //
///   5. Assert that the prev leaf's next points to the leaf being deleted (label match) //
///   6. Verify the merkle proof for the leaf to be deleted //
///   7. Verify the merkle proof for the prev leaf //
///   8. Update the leaf to be deleted to inactive state //
///   9. Update the prev leaf's next to point to the deleted leaf's next //
///   10. Ensure that the final state's count is updated correctly (decremented by 1) //
///   11. Verify the merkle root hash consistency across the updates //
///   12. Update the priority if needed:
///      - If the deleted leaf was the first leaf in the tree (had the highest priority)
///      - The new priority will be the next leaf's price and time
///   13. Yield the results by adding values to ProcessorLookupElements
///   14. Finalize the evaluation
